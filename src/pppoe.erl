...
tag(?VENDOR_SPECIFIC) ->
    'Vendor-Specific';
tag(?RELAY_SESSION_ID) ->
    'Relay-Session-Id';
tag(?SERVICE_NAME_ERROR) ->
    'Service-Name-Error';
tag(?AC_SYSTEM_ERROR) ->
    'AC-System-Error';
tag(?GENERIC_ERROR) ->
...
tag('Relay-Session-Id') ->
    ?RELAY_SESSION_ID;
tag('Service-Name-Error') ->
    ?SERVICE_NAME_ERROR;
tag('AC-System-Error') ->
    ?AC_SYSTEM_ERROR;
tag('Generic-Error') ->
    ?GENERIC_ERROR;
tag(X) when is_integer(X) ->
    X.
...
pppoe_code(?PPPOE_PPP) ->
    ppp;
pppoe_code(?PPPOE_PADO) ->
    pado;
pppoe_code(?PPPOE_PADI) ->
    padi;
pppoe_code(?PPPOE_PADR) ->
    padr;
pppoe_code(?PPPOE_PADS) ->
    pads;
pppoe_code(?PPPOE_PADT) ->
    padt;
pppoe_code(ppp) ->
    ?PPPOE_PPP;
pppoe_code(pado) ->
...
decode_tlv(<<0:16/integer, 0:16/integer>>, Acc) ->
    ...
    lists:reverse(
        lists:flatten(Acc));
decode_tlv(<<Tag:16/integer, Length:16/integer, Value:Length/binary, Rest/binary>>,
           Acc) ->
