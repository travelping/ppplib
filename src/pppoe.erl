...
dslf_tag(138) -> 'Minimum-Data-Rate-Downstream-Low-Power';
dslf_tag(139) -> 'Maximum-Interleaving-Delay-Upstream';
dslf_tag(140) -> 'Actual-Interleaving-Delay-Upstream';
dslf_tag(141) -> 'Maximum-Interleaving-Delay-Downstream';
...
decode(
        <<
                ...
                Type:4/integer,
                Code:8/integer,
                SessionId:16/integer,
                Length:16/integer,
                PayLoad:Length/binary,
                _Rest/binary
        >>
)
when ?PPPOE_VERSION == Version, ?PPPOE_TYPE == Type ->
        decode(pppoe_code(Code), SessionId, PayLoad).

decode(ppp, SessionId, PayLoad) -> {ppp, SessionId, PayLoad};
decode(Code, SessionId, PayLoad) -> {Code, SessionId, decode_tlv(PayLoad)}.
...
decode_tag(<<Vendor:32/integer, Value/binary>>, 'Vendor-Specific') when Vendor == 3561 ->
        decode_dslf_tags(Value, []);

decode_tag(<<Vendor:32/integer, Value/binary>>, Tag = 'Vendor-Specific') -> {Tag, Vendor, Value};
decode_tag(Value, Tag) -> {Tag, Value}.

decode_tlv(<<>>, Acc) -> lists:reverse(lists:flatten(Acc));
...
when Code == pado; Code == padi; Code == padr; Code == pads; Code == padt ->
...
encode_tlv([TLVs | T], B) when is_list(TLVs) -> encode_tlv(T, encode_tlv(TLVs, B)).
