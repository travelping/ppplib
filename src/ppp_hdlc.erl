...
-export([crc16/1, decapsulate/1, encapsulate/1]).

-ignore_xref([{decapsulate, 1},
              {encapsulate, 1},
              {crc16, 1}]).
...
decapsulate(Data) ->
    decapsulate_frames(binary:split(Data,
                                    <<126>>,
                                    [global]),
                       []).
...
decapsulate_frames([HDLCData | Rest], Acc) ->
    ...
    <<CRCData:DataLen/bytes, CRC:16/little-integer>> =
        Frame,
    case crc16(CRCData) bxor 65535 of
        CRC ->
            <<Address:8/integer, Control:8/integer,
              PayLoad/bytes>> =
                CRCData,
        ...
        MyCRC ->
            ?LOG(debug,
                 "invalid HDLC CRC, wanted ~.16.0B, got "
                 "~.16.0B",
                 [MyCRC, CRC]),
            decapsulate_frames(Rest, Acc)
    end.
...
encapsulate(Frames) ->
    <<<< <<126:8, (encapsulate_frame(F))/binary>>
          || F <- Frames >>/binary,
      126:8>>.

encapsulate_frame({Address, Control, Data}) ->
    CRC = crc16(<<Address:8, Control:8, Data/binary>>) bxor
              65535,
    encap(<<Address:8, Control:8, Data/binary,
            CRC:16/little-integer>>,
          <<>>).

decap(<<>>, Acc) -> Acc;
decap(<<125, Char:8/integer, Rest/binary>>, Acc) ->
    decap(Rest, <<Acc/binary, (Char bxor 32):8>>);
