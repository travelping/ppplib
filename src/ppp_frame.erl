...
-define(PPP_EAP, 49703).

%% Extensible Authentication Protocol

-define(IPCP_VJC_COMP, 45).

%% Van Jacobson Compressed TCP/IP		[RFC1144][RFC1332]

-define(IPCP_IPH_COMP, 97).

%% Robust Header Compression (ROHC) 		[RFC3241]

-define(IPCP_ROG_COMP, 3).

%% IP Header Compression			[RFC2507][RFC3544]

-define('CP-VendorSpecific', 0).
-define('CP-Configure-Request', 1).

%% [RFC1661]

-define('CP-Configure-Ack', 2).

%% [RFC1661]

-define('CP-Configure-Nak', 3).

%% [RFC1661]

-define('CP-Configure-Reject', 4).

%% [RFC1661]

-define('CP-Terminate-Request', 5).

%% [RFC1661]

-define('CP-Terminate-Ack', 6).

%% [RFC1661]

-define('CP-Code-Reject', 7).

%% [RFC1661]

-define('CP-Protocol-Reject', 8).

%% [RFC1661]

-define('CP-Echo-Request', 9).

%% [RFC1661]

-define('CP-Echo-Reply', 10).

%% [RFC1661]

-define('CP-Discard-Request', 11).

%% [RFC1661]

-define('CP-Identification', 12).

%% [RFC1570]

-define('CP-Time-Remaining', 13).

%% [RFC1570]

-define('CP-Reset-Request', 14).

%% [RFC1962]

-define('CP-Reset-Reply', 15).
...
%% Magic Number						[RFC1661]

-define(CI_PCOMPRESSION, 7).

%% Protocol Field Compression				[RFC1661]

-define(CI_ACCOMPRESSION, 8).

%% Address/Control Field Compression			[RFC1661]
...
%% max reconstructed receive unit; multilink		[RFC1990]

-define(CI_SSNHF, 18).

%% short sequence numbers for multilink			[RFC1990]

-define(CI_EPDISC, 19).

%% endpoint discriminator				[RFC1990]
...
%% IP Addresses				[RFC1332]

-define(CI_COMPRESSTYPE, 2).

%% Compression Type			[RFC1332]
...
%% Primary DNS value			[RFC1877]

-define(CI_MS_WINS1, 130).

%% Primary WINS value			[RFC1877]

-define(CI_MS_DNS2, 131).

%% Secondary DNS value			[RFC1877]

-define(CI_MS_WINS2, 132).

%% Secondary WINS value			[RFC1877]

-define(CI_IFACEID, 1).

%% Interface-Identifier			[RFC5072]

-define(CI_IPV6_COMPRESSTYPE, 2).

%% IPv6-Compression-Protocol		[RFC5172]
...
decode(<<?PPP_IP:8/integer, Info/binary>>) -> {ipv4, Info};
decode(<<?PPP_IP:16/integer, Info/binary>>) -> {ipv4, Info};
decode(<<?PPP_IPV6:8/integer, Info/binary>>) -> {ipv6, Info};
decode(<<?PPP_IPV6:16/integer, Info/binary>>) -> {ipv6, Info};
...
encode({lcp, Code, Id, Data}) when Code == 'CP-Terminate-Request'; Code == 'CP-Terminate-Ack' ->
        encode(<<?PPP_LCP : 16>>, cp_code(Code), Id, Data);
...
encode({lcp, 'CP-Protocol-Reject', Id, RejectedProtocol, RejectedInfo}) ->
        encode(
                ...
                ?'CP-Protocol-Reject',
                Id,
                <<(protocol(RejectedProtocol)) : 16, RejectedInfo/binary>>
        );
...
encode({lcp, 'CP-Time-Remaining', Id, Magic, Remaining, Message}) ->
        encode(
                ...
                ?'CP-Time-Remaining',
                Id,
                <<Magic : 32, Remaining : 32, Message/binary>>
        );
...
encode({pap, Code, Id, Msg}) when Code == 'PAP-Authenticate-Ack'; Code == 'PAP-Authenticate-Nak' ->
        MsgLength = size(Msg),
        encode(<<?PPP_PAP : 16>>, pap_code(Code), Id, <<MsgLength : 8, Msg/binary>>);

encode({chap, Code, Id, Value, Name}) when Code == 'CHAP-Challenge'; Code == 'CHAP-Response' ->
...
encode({chap, Code, Id, Msg}) when Code == 'CHAP-Success'; Code == 'CHAP-Failure' ->
...
encode({ipcp, Code, Id, Data}) when Code == 'CP-Terminate-Request'; Code == 'CP-Terminate-Ack' ->
        encode(<<?PPP_IPCP : 16>>, cp_code(Code), Id, Data);
...
encode({ipv6cp, Code, Id, Data}) when Code == 'CP-Terminate-Request'; Code == 'CP-Terminate-Ack' ->
        encode(<<?PPP_IPV6CP : 16>>, cp_code(Code), Id, Data);
...
encode({Protocol, Code, Id, Data}) when Protocol >= 32768 ->
...
decode_lcp(Data, Id, Code) when Code == ?'CP-Terminate-Request'; Code == ?'CP-Terminate-Ack' ->
        {lcp, cp_code(Code), Id, Data};

decode_lcp(RejectedPacket, Id, ?'CP-Code-Reject') -> {lcp, 'CP-Code-Reject', Id, RejectedPacket};
decode_lcp(_Data, Id, ?'CP-Discard-Request') -> {lcp, 'CP-Discard-Request', Id};
...
when Code == ?'PAP-Authenticate-Ack'; Code == ?'PAP-Authenticate-Nak' ->
        <<Msg:MsgLength/bytes, _/binary>> = Rest,
        {pap, pap_code(Code), Id, Msg}.


decode_chap(Msg, Id, Code) when Code == ?'CHAP-Success'; Code == ?'CHAP-Failure' ->
        {chap, chap_code(Code), Id, Msg};

decode_chap(<<ValueSize:8/integer, Rest/binary>>, Id, Code)
when Code == ?'CHAP-Challenge'; Code == ?'CHAP-Response' ->
...
decode_ipcp(Data, Id, Code) when Code == ?'CP-Terminate-Request'; Code == ?'CP-Terminate-Ack' ->
        {ipcp, cp_code(Code), Id, Data};

decode_ipcp(RejectedPacket, Id, ?'CP-Code-Reject') -> {ipcp, 'CP-Code-Reject', Id, RejectedPacket};
...
decode_ipv6cp(Data, Id, Code) when Code == ?'CP-Terminate-Request'; Code == ?'CP-Terminate-Ack' ->
...
decode_lcp_option(<<MRU:16/integer>>, ?CI_MRU) -> {mru, MRU};
decode_lcp_option(<<ACCM:32/integer>>, ?CI_ASYNCMAP) -> {asyncmap, ACCM};

decode_lcp_option(<<Auth:16/integer>>, ?CI_AUTHTYPE) when Auth == ?PPP_EAP; Auth == ?PPP_PAP ->
        {auth, cp_auth_protocol(Auth)};

decode_lcp_option(<<Auth:16/integer, MDType:8/integer>>, ?CI_AUTHTYPE) when Auth == ?PPP_CHAP ->
        {auth, {cp_auth_protocol(Auth), chap_md_type(MDType)}};

decode_lcp_option(<<QP:16/integer, Period:32/integer>>, ?CI_QUALITY) -> {quality, QP, Period};
decode_lcp_option(<<Magic:32/integer>>, ?CI_MAGICNUMBER) -> {magic, Magic};
decode_lcp_option(<<>>, ?CI_PCOMPRESSION) -> pfc;
decode_lcp_option(<<>>, ?CI_ACCOMPRESSION) -> acfc;
decode_lcp_option(<<Op:8/integer, Message/binary>>, ?CI_CALLBACK) -> {callback, Op, Message};
decode_lcp_option(<<MRRU:16/integer>>, ?CI_MRRU) -> {mrru, MRRU};
decode_lcp_option(<<>>, ?CI_SSNHF) -> ssnhf;
decode_lcp_option(<<Class:8/integer, Address/binary>>, ?CI_EPDISC) -> {epdisc, Class, Address};
decode_lcp_option(<<LDisc:16/integer>>, ?CI_LDISC) -> {ldisc, LDisc};
decode_lcp_option(Data, Type) -> {Type, Data}.

decode_lcp_options(Options) -> decode_lcp_options(Options, []).

decode_lcp_options(<<>>, Acc) -> lists:reverse(Acc);
...
decode_ipcp_option(Addresses, ?CI_ADDRS) -> {addresses, Addresses};

decode_ipcp_option(
        <<?PPP_VJC_COMP:16/integer, MaxSlotId:8/integer, CompSlotId:8/integer>>,
        ?CI_COMPRESSTYPE
) ->
        {compresstype, vjc, MaxSlotId, CompSlotId};

decode_ipcp_option(<<Addr:4/bytes>>, ?CI_ADDR) -> {addr, Addr};
decode_ipcp_option(<<Addr:4/bytes>>, ?CI_MS_DNS1) -> {ms_dns1, Addr};
decode_ipcp_option(<<Addr:4/bytes>>, ?CI_MS_WINS1) -> {ms_wins1, Addr};
decode_ipcp_option(<<Addr:4/bytes>>, ?CI_MS_DNS2) -> {ms_dns2, Addr};
decode_ipcp_option(<<Addr:4/bytes>>, ?CI_MS_WINS2) -> {ms_wins2, Addr};
decode_ipcp_option(Data, Type) -> {Type, Data}.

decode_ipcp_options(Options) -> decode_ipcp_options(Options, []).

decode_ipcp_options(<<>>, Acc) -> lists:reverse(Acc);
%% variable length options decoding
...
decode_ipv6cp_option(IFaceId, ?CI_IFACEID) -> {ifaceid, IFaceId};

decode_ipv6cp_option(<<?PPP_IPV6CP_COMP:16/integer>>, ?CI_IPV6_COMPRESSTYPE) ->
        {compresstype, ipv6_hc};

decode_ipv6cp_option(Data, Type) -> {Type, Data}.

decode_ipv6cp_options(Options) -> decode_ipv6cp_options(Options, []).

decode_ipv6cp_options(<<>>, Acc) -> lists:reverse(Acc);
%% variable length options decoding
...
encode_lcp_option([]) -> <<>>;
encode_lcp_option({mru, MRU}) -> encode_lcp_option(?CI_MRU, <<MRU:16/integer>>);
encode_lcp_option({asyncmap, ACCM}) -> encode_lcp_option(?CI_ASYNCMAP, <<ACCM:32/integer>>);

encode_lcp_option({auth, Auth}) when Auth == eap; Auth == pap ->
        encode_lcp_option(?CI_AUTHTYPE, <<(cp_auth_protocol(Auth)) : 16/integer>>);

encode_lcp_option({auth, {Auth, MDType}}) when Auth == chap ->
        encode_lcp_option(
                ?CI_AUTHTYPE,
                <<(cp_auth_protocol(Auth)) : 16/integer, (chap_md_type(MDType)) : 8/integer>>
        );

encode_lcp_option({quality, QP, Period}) ->
        encode_lcp_option(?CI_QUALITY, <<QP:16/integer, Period:32/integer>>);

encode_lcp_option({magic, Magic}) -> encode_lcp_option(?CI_MAGICNUMBER, <<Magic:32/integer>>);
encode_lcp_option(pfc) -> encode_lcp_option(?CI_PCOMPRESSION, <<>>);
encode_lcp_option(acfc) -> encode_lcp_option(?CI_ACCOMPRESSION, <<>>);

encode_lcp_option({callback, Op, Message}) ->
        encode_lcp_option(?CI_CALLBACK, <<Op:8/integer, Message/binary>>);

encode_lcp_option({mrru, MRRU}) -> encode_lcp_option(?CI_MRRU, <<MRRU:16/integer>>);
encode_lcp_option(ssnhf) -> encode_lcp_option(?CI_SSNHF, <<>>);

encode_lcp_option({epdisc, Class, Address}) ->
        encode_lcp_option(?CI_EPDISC, <<Class:8/integer, Address/binary>>);

encode_lcp_option({ldisc, LDisc}) -> encode_lcp_option(?CI_LDISC, <<LDisc:16/integer>>);
encode_lcp_option({Type, Data}) -> encode_lcp_option(Type, Data).

encode_lcp_options(Options) -> <<<<(encode_lcp_option(O))/binary>> || O <- Options>>.
...
encode_ipcp_option([]) -> <<>>;
encode_ipcp_option({addresses, Addresses}) -> encode_ipcp_option(?CI_ADDRS, Addresses);
...
encode_ipcp_option({addr, Addr}) -> encode_ipcp_option(?CI_ADDR, Addr);
encode_ipcp_option({ms_dns1, Addr}) -> encode_ipcp_option(?CI_MS_DNS1, Addr);
encode_ipcp_option({ms_wins1, Addr}) -> encode_ipcp_option(?CI_MS_WINS1, Addr);
encode_ipcp_option({ms_dns2, Addr}) -> encode_ipcp_option(?CI_MS_DNS2, Addr);
encode_ipcp_option({ms_wins2, Addr}) -> encode_ipcp_option(?CI_MS_WINS2, Addr);
encode_ipcp_option({Type, Data}) -> encode_ipcp_option(Type, Data).

encode_ipcp_options(Options) -> <<<<(encode_ipcp_option(O))/binary>> || O <- Options>>.
...
encode_ipv6cp_option({ifaceid, IFaceId}) -> encode_ipv6cp_option(?CI_IFACEID, IFaceId);
...
encode_ipv6cp_options(Options) -> <<<<(encode_ipv6cp_option(O))/binary>> || O <- Options>>.
