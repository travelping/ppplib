...
-define(PPP_AT, 16#29).
%% IPX protocol
-define(PPP_IPX, 16#2b).
%% VJ compressed TCP
-define(PPP_VJC_COMP, 16#2d).
%% VJ uncompressed TCP
-define(PPP_VJC_UNCOMP, 16#2f).
-define(PPP_IPV6CP_COMP, 16#004f).
%% Internet Protocol Version 6
-define(PPP_IPV6, 16#57).
%% compressed packet
-define(PPP_COMP, 16#fd).
%% IP Control Protocol
-define(PPP_IPCP, 16#8021).
%% AppleTalk Control Protocol
-define(PPP_ATCP, 16#8029).
%% IPX Control Protocol
-define(PPP_IPXCP, 16#802b).
%% IPv6 Control Protocol
-define(PPP_IPV6CP, 16#8057).
%% Compression Control Protocol
-define(PPP_CCP, 16#80fd).
%% Encryption Control Protocol
-define(PPP_ECP, 16#8053).
%% Link Control Protocol
-define(PPP_LCP, 16#c021).
%% Password Authentication Protocol
-define(PPP_PAP, 16#c023).
%% Link Quality Report protocol
-define(PPP_LQR, 16#c025).
%% Cryptographic Handshake Auth. Protocol
-define(PPP_CHAP, 16#c223).
%% Callback Control Protocol
-define(PPP_CBCP, 16#c029).
%% Extensible Authentication Protocol
-define(PPP_EAP, 16#c227).

%% Van Jacobson Compressed TCP/IP		[RFC1144][RFC1332]
-define(IPCP_VJC_COMP, 16#2d).
%% Robust Header Compression (ROHC) 		[RFC3241]
-define(IPCP_IPH_COMP, 16#61).
%% IP Header Compression			[RFC2507][RFC3544]
-define(IPCP_ROG_COMP, 16#03).
...
decode(<<Protocol:16/integer, Code:8/integer, Id:8/integer, _:16/integer, Data/binary>>) when
    Protocol >= 16#8000
->
...
encode({lcp, Code, Id, Options}) when
    Code == 'CP-Configure-Request';
...
encode({lcp, Code, Id, Data}) when
    Code == 'CP-Terminate-Request';
...
encode({pap, Code, Id, Msg}) when
    Code == 'PAP-Authenticate-Ack';
...
encode({chap, Code, Id, Value, Name}) when
    Code == 'CHAP-Challenge';
    Code == 'CHAP-Response'
->
...
encode({chap, Code, Id, Msg}) when
    Code == 'CHAP-Success';
    Code == 'CHAP-Failure'
->
    encode(<<?PPP_CHAP:16>>, chap_code(Code), Id, Msg);
encode({ipcp, Code, Id, Options}) when
    Code == 'CP-Configure-Request';
    Code == 'CP-Configure-Ack';
    Code == 'CP-Configure-Nak';
    Code == 'CP-Configure-Reject'
...
encode({ipv6cp, Code, Id, Options}) when
    Code == 'CP-Configure-Request';
    Code == 'CP-Configure-Ack';
    Code == 'CP-Configure-Nak';
    Code == 'CP-Configure-Reject'
...
encode({ipv6cp, Code, Id, Data}) when
    Code == 'CP-Terminate-Request';
    Code == 'CP-Terminate-Ack'
->
    encode(<<?PPP_IPV6CP:16>>, cp_code(Code), Id, Data);
...
decode_pap(<<MsgLength:8/integer, Rest/binary>>, Id, Code) when
    Code == ?'PAP-Authenticate-Ack';
    Code == ?'PAP-Authenticate-Nak'
->
...
decode_chap(Msg, Id, Code) when
    Code == ?'CHAP-Success';
    Code == ?'CHAP-Failure'
->
    {chap, chap_code(Code), Id, Msg};
decode_chap(<<ValueSize:8/integer, Rest/binary>>, Id, Code) when
    Code == ?'CHAP-Challenge';
    Code == ?'CHAP-Response'
->
...
decode_ipv6cp(Data, Id, Code) when
    Code == ?'CP-Configure-Request';
    Code == ?'CP-Configure-Ack';
    Code == ?'CP-Configure-Nak';
    Code == ?'CP-Configure-Reject'
