...
-define(IPCP_VJC_COMP,
        16#2d).        %% Van Jacobson Compressed TCP/IP                [RFC1144][RFC1332]
-define(IPCP_IPH_COMP,
        16#61).        %% Robust Header Compression (ROHC)                 [RFC3241]
-define(IPCP_ROG_COMP,
        16#03).        %% IP Header Compression                        [RFC2507][RFC3544]
...
cp_code(?'CP-VendorSpecific') ->
    'CP-VendorSpecific';
cp_code(?'CP-Configure-Request') ->
    'CP-Configure-Request';
cp_code(?'CP-Configure-Ack') ->
    'CP-Configure-Ack';
cp_code(?'CP-Configure-Nak') ->
    'CP-Configure-Nak';
cp_code(?'CP-Configure-Reject') ->
    'CP-Configure-Reject';
cp_code(?'CP-Terminate-Request') ->
    'CP-Terminate-Request';
cp_code(?'CP-Terminate-Ack') ->
    'CP-Terminate-Ack';
cp_code(?'CP-Code-Reject') ->
    'CP-Code-Reject';
cp_code(?'CP-Protocol-Reject') ->
    'CP-Protocol-Reject';
cp_code(?'CP-Echo-Request') ->
    'CP-Echo-Request';
cp_code(?'CP-Echo-Reply') ->
    'CP-Echo-Reply';
cp_code(?'CP-Discard-Request') ->
    'CP-Discard-Request';
cp_code(?'CP-Identification') ->
    'CP-Identification';
cp_code(?'CP-Time-Remaining') ->
    'CP-Time-Remaining';
cp_code(?'CP-Reset-Request') ->
    'CP-Reset-Request';
cp_code(?'CP-Reset-Reply') ->
    'CP-Reset-Reply';
cp_code('CP-VendorSpecific') ->
    ?'CP-VendorSpecific';
cp_code('CP-Configure-Request') ->
...
protocol(?PPP_CHAP) ->
    chap;
protocol(X) when is_integer(X) ->
    X.

cp_auth_protocol(?PPP_PAP) ->
    pap;
cp_auth_protocol(?PPP_CHAP) ->
    chap;
cp_auth_protocol(?PPP_EAP) ->
    eap;
cp_auth_protocol(pap) ->
    ?PPP_PAP;
cp_auth_protocol(chap) ->
    ?PPP_CHAP;
cp_auth_protocol(eap) ->
    ?PPP_EAP.
...
chap_md_type(sha1) ->
    6;
chap_md_type('MS-CHAP') ->
    128;
chap_md_type('MS-CHAP-v2') ->
    129;
chap_md_type(5) ->
    md5;
chap_md_type(6) ->
...
chap_code(?'CHAP-Challenge') ->
    'CHAP-Challenge';
chap_code(?'CHAP-Response') ->
    'CHAP-Response';
chap_code(?'CHAP-Success') ->
    'CHAP-Success';
chap_code(?'CHAP-Failure') ->
    'CHAP-Failure';
chap_code('CHAP-Challenge') ->
    ?'CHAP-Challenge';
chap_code('CHAP-Response') ->
...
encode({lcp, Code, Id, Data})
    when Code == 'CP-Terminate-Request'; Code == 'CP-Terminate-Ack' ->
    encode(<<?PPP_LCP:16>>, cp_code(Code), Id, Data);
...
encode({chap, Code, Id, Value, Name})
    ...
    encode(<<?PPP_CHAP:16>>, chap_code(Code), Id, <<ValueSize:8, Value/binary, Name/binary>>);
...
encode({ipv6cp, 'CP-Protocol-Reject', Id, RejectedProtocol, RejectedInfo}) ->
    encode(<<?PPP_IPV6CP:16>>,
           ...
           Id,
           <<(protocol(RejectedProtocol)):16, RejectedInfo/binary>>);
encode({Protocol, Code, Id, Data}) when Protocol >= 16#8000 ->
...
decode_lcp(<<RejectedProtocol:16/integer, RejectedInfo/binary>>,
           Id,
           ?'CP-Protocol-Reject') ->
...
decode_lcp(<<Magic:32/integer, Remaining:32/integer, Message/binary>>,
           Id,
           ?'CP-Time-Remaining') ->
...
decode_ipcp(<<RejectedProtocol:16/integer, RejectedInfo/binary>>,
            Id,
            ?'CP-Protocol-Reject') ->
...
decode_ipv6cp(<<RejectedProtocol:16/integer, RejectedInfo/binary>>,
              Id,
              ?'CP-Protocol-Reject') ->
