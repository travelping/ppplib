...
-compile({inline,
          [{decode_lcp, 3},
           {decode_lcp_option, 2},
           {decode_ipcp_option, 2}]}).

-define(PPP_IP, 33).        %% Internet Protocol

-define(PPP_AT, 41).        %% AppleTalk Protocol

-define(PPP_IPX, 43).        %% IPX protocol

-define(PPP_VJC_COMP, 45).        %% VJ compressed TCP

-define(PPP_VJC_UNCOMP,
        47).        %% VJ uncompressed TCP
...
-define(PPP_COMP, 253).        %% compressed packet
...
-define(PPP_IPXCP, 32811).      %% IPX Control Protocol
...
-define(PPP_LCP, 49185).      %% Link Control Protocol

-define(PPP_PAP,
        49187).      %% Password Authentication Protocol

-define(PPP_LQR,
        49189).      %% Link Quality Report protocol

-define(PPP_CHAP,
        49699).      %% Cryptographic Handshake Auth. Protocol

-define(PPP_CBCP,
        49193).      %% Callback Control Protocol

-define(PPP_EAP,
        49703).      %% Extensible Authentication Protocol

-define(IPCP_VJC_COMP,
        45).        %% Van Jacobson Compressed TCP/IP		[RFC1144][RFC1332]

-define(IPCP_IPH_COMP,
        97).        %% Robust Header Compression (ROHC) 		[RFC3241]

-define(IPCP_ROG_COMP,
        3).        %% IP Header Compression			[RFC2507][RFC3544]

-define('CP-VendorSpecific', 0).

-define('CP-Configure-Request',
        1).                                                     %% [RFC1661]

-define('CP-Configure-Ack',
        2).                                                     %% [RFC1661]

-define('CP-Configure-Nak',
        3).                                                     %% [RFC1661]

-define('CP-Configure-Reject',
        4).                                                     %% [RFC1661]

-define('CP-Terminate-Request',
        5).                                                     %% [RFC1661]

-define('CP-Terminate-Ack',
        6).                                                     %% [RFC1661]

-define('CP-Code-Reject',
        7).                                                     %% [RFC1661]

-define('CP-Protocol-Reject',
        8).                                                     %% [RFC1661]

-define('CP-Echo-Request',
        9).                                                     %% [RFC1661]

-define('CP-Echo-Reply',
        10).                                                     %% [RFC1661]

-define('CP-Discard-Request',
        11).                                                     %% [RFC1661]

-define('CP-Identification',
        12).                                                     %% [RFC1570]

-define('CP-Time-Remaining',
        13).                                                     %% [RFC1570]

-define('CP-Reset-Request',
        14).                                                     %% [RFC1962]

-define('CP-Reset-Reply',
        15).                                                     %% [RFC1962]

cp_code(?'CP-VendorSpecific') -> 'CP-VendorSpecific';
cp_code(?'CP-Configure-Request') ->
    'CP-Configure-Request';
cp_code(?'CP-Configure-Ack') -> 'CP-Configure-Ack';
cp_code(?'CP-Configure-Nak') -> 'CP-Configure-Nak';
cp_code(?'CP-Configure-Reject') ->
    'CP-Configure-Reject';
cp_code(?'CP-Terminate-Request') ->
    'CP-Terminate-Request';
...
-define(CI_ACCOMPRESSION,
        8).   %% Address/Control Field Compression			[RFC1661]

-define(CI_CALLBACK,
        13).   %% callback						[RFC1570]

-define(CI_MRRU,
        17).   %% max reconstructed receive unit; multilink		[RFC1990]

-define(CI_SSNHF,
        18).   %% short sequence numbers for multilink			[RFC1990]
...
-define(CI_ADDRS,
        1).             %% IP Addresses				[RFC1332]

-define(CI_COMPRESSTYPE,
        2).             %% Compression Type			[RFC1332]

-define(CI_ADDR,
        3).             %% IP-Address				[RFC1332]

-define(CI_MS_DNS1,
        129).           %% Primary DNS value			[RFC1877]

-define(CI_MS_WINS1,
        130).           %% Primary WINS value			[RFC1877]

-define(CI_MS_DNS2,
        131).           %% Secondary DNS value			[RFC1877]

-define(CI_MS_WINS2,
        132).           %% Secondary WINS value			[RFC1877]

-define(CI_IFACEID,
        1).             %% Interface-Identifier			[RFC5072]

-define(CI_IPV6_COMPRESSTYPE,
...
pap_code(?'PAP-Authentication-Request') ->
    'PAP-Authentication-Request';
pap_code(?'PAP-Authenticate-Ack') ->
    'PAP-Authenticate-Ack';
pap_code(?'PAP-Authenticate-Nak') ->
    'PAP-Authenticate-Nak';
pap_code('PAP-Authentication-Request') ->
    ?'PAP-Authentication-Request';
pap_code('PAP-Authenticate-Ack') ->
    ?'PAP-Authenticate-Ack';
...
decode(<<(?PPP_IPV6):16/integer, Info/binary>>) ->
    {ipv6, Info};
decode(<<(?PPP_IPCP):16/integer, Code:8/integer,
         Id:8/integer, Length:16/integer, Rest/binary>>) ->
    DataLen = Length - 4,
    <<Data:DataLen/bytes, _Pad/binary>> = Rest,
    decode_ipcp(Data, Id, Code);
decode(<<(?PPP_IPV6CP):16/integer, Code:8/integer,
         Id:8/integer, Length:16/integer, Rest/binary>>) ->
    DataLen = Length - 4,
    <<Data:DataLen/bytes, _Pad/binary>> = Rest,
    decode_ipv6cp(Data, Id, Code);
decode(<<(?PPP_LCP):16/integer, Code:8/integer,
         Id:8/integer, Length:16/integer, Rest/binary>>) ->
    DataLen = Length - 4,
    <<Data:DataLen/bytes, _Pad/binary>> = Rest,
    decode_lcp(Data, Id, Code);
decode(<<(?PPP_PAP):16/integer, Code:8/integer,
         Id:8/integer, Length:16/integer, Rest/binary>>) ->
    DataLen = Length - 4,
    <<Data:DataLen/bytes, _Pad/binary>> = Rest,
    decode_pap(Data, Id, Code);
decode(<<(?PPP_CHAP):16/integer, Code:8/integer,
         Id:8/integer, Length:16/integer, Rest/binary>>) ->
    DataLen = Length - 4,
    <<Data:DataLen/bytes, _Pad/binary>> = Rest,
    decode_chap(Data, Id, Code);
decode(<<Protocol:16/integer, Code:8/integer,
         Id:8/integer, _:16/integer, Data/binary>>)
    when Protocol >= 32768 ->
...
encode({pap,
    ...
    encode(<<(?PPP_PAP):16>>,
           ?'PAP-Authentication-Request',
           Id,
           <<PeerLength:8, PeerId/binary, PassLength:8,
             Passwd/binary>>);
...
encode({chap, Code, Id, Msg})
    when Code == 'CHAP-Success'; Code == 'CHAP-Failure' ->
...
decode_lcp(<<Magic:32/integer, _Data/binary>>, Id,
           ?'CP-Echo-Request') ->
    {lcp, 'CP-Echo-Request', Id, Magic};
decode_lcp(<<Magic:32/integer, _Data/binary>>, Id,
           ?'CP-Echo-Reply') ->
    {lcp, 'CP-Echo-Reply', Id, Magic};
decode_lcp(<<RejectedProtocol:16/integer,
             RejectedInfo/binary>>,
           Id, ?'CP-Protocol-Reject') ->
    {lcp,
     'CP-Protocol-Reject',
     Id,
     protocol(RejectedProtocol),
     RejectedInfo};
decode_lcp(<<Magic:32/integer, Message/binary>>, Id,
           ?'CP-Identification') ->
    {lcp, 'CP-Identification', Id, Magic, Message};
decode_lcp(<<Magic:32/integer, Remaining:32/integer,
             Message/binary>>,
           Id, ?'CP-Time-Remaining') ->
    {lcp,
     'CP-Time-Remaining',
     Id,
     Magic,
...
decode_pap(<<PeerLength:8/integer, Rest/binary>>, Id,
           ?'PAP-Authentication-Request') ->
    <<PeerId:PeerLength/bytes, PassLength:8/integer,
      More/binary>> =
        Rest,
    <<Passwd:PassLength/bytes, _/binary>> = More,
    {pap, 'PAP-Authentication-Request', Id, PeerId, Passwd};
decode_pap(<<MsgLength:8/integer, Rest/binary>>, Id,
           Code)
    when Code == (?'PAP-Authenticate-Ack');
         Code == (?'PAP-Authenticate-Nak') ->
...
decode_chap(<<ValueSize:8/integer, Rest/binary>>, Id,
            Code)
    when Code == (?'CHAP-Challenge');
         Code == (?'CHAP-Response') ->
...
decode_ipcp(<<RejectedProtocol:16/integer,
              RejectedInfo/binary>>,
            Id, ?'CP-Protocol-Reject') ->
    {ipcp,
     'CP-Protocol-Reject',
...
decode_ipv6cp(<<RejectedProtocol:16/integer,
                RejectedInfo/binary>>,
              Id, ?'CP-Protocol-Reject') ->
    {ipv6cp,
     'CP-Protocol-Reject',
...
decode_lcp_option(<<Auth:16/integer, MDType:8/integer>>,
                  ?CI_AUTHTYPE)
    when Auth == (?PPP_CHAP) ->
    {auth, {cp_auth_protocol(Auth), chap_md_type(MDType)}};
decode_lcp_option(<<QP:16/integer, Period:32/integer>>,
                  ?CI_QUALITY) ->
    {quality, QP, Period};
decode_lcp_option(<<Magic:32/integer>>,
                  ?CI_MAGICNUMBER) ->
    {magic, Magic};
decode_lcp_option(<<>>, ?CI_PCOMPRESSION) -> pfc;
decode_lcp_option(<<>>, ?CI_ACCOMPRESSION) -> acfc;
decode_lcp_option(<<Op:8/integer, Message/binary>>,
                  ?CI_CALLBACK) ->
    {callback, Op, Message};
decode_lcp_option(<<MRRU:16/integer>>, ?CI_MRRU) ->
    {mrru, MRRU};
decode_lcp_option(<<>>, ?CI_SSNHF) -> ssnhf;
decode_lcp_option(<<Class:8/integer, Address/binary>>,
                  ?CI_EPDISC) ->
...
decode_lcp_options(Data = <<Type:8/integer,
                            Length:8/integer, Rest/binary>>,
                   Acc) ->
    PayLoadLen = Length - 2,
    case Rest of
...
decode_ipcp_option(Addresses, ?CI_ADDRS) ->
    {addresses, Addresses};
decode_ipcp_option(<<(?PPP_VJC_COMP):16/integer,
                     MaxSlotId:8/integer, CompSlotId:8/integer>>,
                   ?CI_COMPRESSTYPE) ->
...
%% variable length options decoding
decode_ipcp_options(Data = <<Type:8/integer,
                             Length:8/integer, Rest/binary>>,
                    Acc) ->
    PayLoadLen = Length - 2,
    case Rest of
...
decode_ipv6cp_option(IFaceId, ?CI_IFACEID) ->
    {ifaceid, IFaceId};
decode_ipv6cp_option(<<(?PPP_IPV6CP_COMP):16/integer>>,
                     ?CI_IPV6_COMPRESSTYPE) ->
...
%% variable length options decoding
decode_ipv6cp_options(Data = <<Type:8/integer,
                               Length:8/integer, Rest/binary>>,
                      Acc) ->
    PayLoadLen = Length - 2,
    case Rest of
...
encode_lcp_option({auth, Auth})
    ...
    encode_lcp_option(?CI_AUTHTYPE,
                      <<(cp_auth_protocol(Auth)):16/integer>>);
encode_lcp_option({auth, {Auth, MDType}})
    when Auth == chap ->
    encode_lcp_option(?CI_AUTHTYPE,
                      <<(cp_auth_protocol(Auth)):16/integer,
                        (chap_md_type(MDType)):8/integer>>);
encode_lcp_option({quality, QP, Period}) ->
    encode_lcp_option(?CI_QUALITY,
                      <<QP:16/integer, Period:32/integer>>);
encode_lcp_option({magic, Magic}) ->
    encode_lcp_option(?CI_MAGICNUMBER,
                      <<Magic:32/integer>>);
...
encode_lcp_option({callback, Op, Message}) ->
    encode_lcp_option(?CI_CALLBACK,
                      <<Op:8/integer, Message/binary>>);
...
encode_lcp_option({epdisc, Class, Address}) ->
    encode_lcp_option(?CI_EPDISC,
                      <<Class:8/integer, Address/binary>>);
...
encode_ipcp_option({compresstype,
                    vjc,
                    MaxSlotId,
                    CompSlotId}) ->
    encode_ipcp_option(?CI_COMPRESSTYPE,
                       <<(?PPP_VJC_COMP):16, MaxSlotId:8, CompSlotId:8>>);
...
encode_ipv6cp_option({compresstype, ipv6_hc}) ->
    encode_ipv6cp_option(?CI_IPV6_COMPRESSTYPE,
                         <<(?PPP_IPV6CP_COMP):16>>);
encode_ipv6cp_option({Type, Data})
    when is_integer(Type), is_binary(Data) ->
    encode_ipv6cp_option(Type, Data).

encode_ipv6cp_options(Options) ->
    << <<(encode_ipv6cp_option(O))/binary>>
        || O <- Options >>.
