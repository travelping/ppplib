...
pppoe_padt() ->
        hexstr2bin(
                "11a700000031010300041004e1d80101"
...
pppoe_padi() ->
        hexstr2bin(
                "11090000003101010005737061726301"
...
pppoe_pado() ->
        hexstr2bin(
                "11070000002a010300041004e1d80101"
...
pppoe_padr() ->
        hexstr2bin(
                "11190000003901010005737061726301"
...
pppoe_pads() ->
        hexstr2bin(
                "11650002002201010005737061726301"
                "0300041004e1d8010200093d4f70656e"
                "424e473d00000000000000000000"
        ).

% hexstr2bin
hexstr2bin(S) -> list_to_binary(hexstr2list(S)).

hexstr2list([X, Y | T]) -> [mkint(X) * 16 + mkint(Y) | hexstr2list(T)];
hexstr2list([]) -> [].

mkint(C) when $0 =< C, C =< $9 -> C - $0;
mkint(C) when $A =< C, C =< $F -> C - $A + 10;
mkint(C) when $a =< C, C =< $f -> C - $a + 10.
...
all() -> [test_pppoe_padt, test_pppoe_padi, test_pppoe_pado, test_pppoe_padr, test_pppoe_pads].
